...
class Visualization:
    ...
    def plot_on_grid(
        ...
        # interpolation
        grid_data = griddata(
            np.stack([self.mx, self.my], axis=1),
            data,
            (self.xx, self.yy),
            method="linear",
        )
        ...
        if scatter_only:
            ...
            cbar = self.f.colorbar(scat, ax=self.ax, shrink=0.7)
    ...
    def plot_line(self, idx_1: int, idx_2: int) -> None:
        ...
        self.ax.plot(
            [self.mx[idx_1], self.mx[idx_2]],
            [self.my[idx_1], self.my[idx_2]],
            "k-",
            lw=0.2,
        )
        return self
    ...
    def plot_edge_index(self, edge_index: Any) -> None:
        for i in range(edge_index.shape[1]):
            self.plot_line(edge_index[0, i], edge_index[1, i])
        return self
    ...
    def to_numpy(self) -> np.array:
        ...
        result = result.reshape((int(self.f.bbox.bounds[3]), int(self.f.bbox.bounds[2]), -1))
